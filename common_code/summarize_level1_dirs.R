...
# SignatureAnalyzer signature file copying functions ------

# Copying signature files extracted by SignatureAnalyzer in seed.X/best.run/
# to seed.X/ 

# Signature file copy function for ONE data set directory (e.g. "SBS_set1") 
copy_sa_files_one_dir <- function(a_dir) {
  
  start.here <- file.path(a_dir, "raw_results")
  
  # A full directory path, e.g "indel_set1/raw_results/SignatureAnalyzer.results"
  analysis_names <- 
    file.path(a_dir, "raw_results", "SignatureAnalyzer.results")
  for (analysis_name in analysis_names) {
    ...
    # dataset_path, e.g indel_set1/raw_results/SignatureAnalyzer.results/"Moderate"
    ...
    for (dataset_path in dataset_paths) {
      ...
      for(seed_path in seeds_paths) {
        ...
        # Pass if SignatureAnalyzer best.run/ folder is absent
        path_to_check <- file.path(seed_path, "best.run")
        if (dir.exists(path_to_check) == FALSE) {
          message("Skipping seed_path=", seed_path, "\n")
          next
        }
        
        # Copy signature in ICAMS format
        sig_path <- paste0(seed_path, "/best.run/sa.output.sigs.csv")
        file.copy(from = sig_path,
                  to = paste0(seed_path, "/extracted.signatures.csv"),
                  overwrite = TRUE,
                  copy.date = TRUE)
        
        message(
          "Finished copying SignatureAnalyzer's ",
          "signature extraction CSV file from ", 
          file.path(seed_path, "best.run")," to ", seed_path, "\n")
        message("---------------------")
      } # for (seed_path in seeds_paths)
    } # for (dataset_path in dataset_paths)
  } # for (analysis_name in analysis_names)
...
copy_sa_files_all_level1_dirs <- function(level1_dirs = level1_dirs) {
  lapply(level1_dirs, copy_sa_files_one_dir)
  message("\n=====================")
  message("Finished copying SignatureAnalyzer files in all sub-directories.")
  invisible(return(NULL))
}
...
move_sigpro_files_one_dir <- function(a_dir) {
  ...
  analysis_names <- 
    file.path(a_dir, "raw_results", "SigProfilerExtractor.results")
  for (analysis_name in analysis_names) {
    if(!dir.exists(analysis_name)) {
      message("\n=====================")
      message("\n=====================")
...
summarize_all_level1_dirs <- function()  {
  ...
  copy_sa_files_all_level1_dirs(level1.dirs)
  ...
  readr::write_csv(all.results.for.export, 
                   outpath("supplementary_table_s4.csv"))
  openxlsx::write.xlsx(supplementary_table_s4, 
                      outpath("supplementary_table_s4.xlsx"))
  save(supplementary_table_s4, 
       file = outpath("supplementary_table_s4.Rdata"))
